...
/// Translate `source` into valid Lojban, self-correcting against the
/// validation gate, with optional jbotci tool-use. See the module docs for the
...
pub async fn translate_agentic<C: ToolChat, V: ToolChat>(
    ...
    let request = format!("Translate to Lojban: {}", source.trim());
