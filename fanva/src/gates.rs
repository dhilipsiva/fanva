...
//!
//! The chain is `gerna::parse_checked` → smuni → the wasm-only "official"
//! **camxes** gate (vendored `camxes.js` via JS-interop). gerna implements a
//! fragment of Lojban and is the narrowest gate; camxes is the official
//! grammar, so the success condition is the intersection gerna ∧ smuni ∧
//! camxes (camxes only where the shim is loaded — see [`official_gate`]).
...
pub enum GateError {
    /// gerna rejected the grammar (`NibliError::Syntax`).
    Syntax(String),
    /// smuni rejected the semantics/arity (`NibliError::Semantic`).
    Semantic(String),
    /// The official grammar (`camxes.js`) rejected it. *(wasm-only gate)*
    Official(String),
...
/// success: gerna grammar → smuni semantics. Which call fails determines the
/// [`GateError`] variant: a `parse_checked` failure is always a grammar error;
/// a `compile_from_gerna_ast` failure is a semantic one.
...
/// The full validation gate the agent calls: [`local_gates`] plus, on wasm,
/// the official (`camxes.js`) grammar gate.
...
/// shim that `fanva-ui` loads. wasm-only and **synchronous** (a plain JS call).
///
/// Degrades to `Ok` when the shim is not present — on native, or if the script
/// failed to load — so the engine stays usable without it and translation is
...
/// Validate a multi-line KB the way `fanva-ui` uses it: each non-empty,
...
/// fix in Lojban only. Kept in one place so the phrasing is consistent across
/// gates.
...
mod tests {
    ...
    #[test]
    fn feedback_carries_the_verifier_issues() {
        let fb = feedback_for(&GateError::Verification("1. off".into()));
        assert!(fb.contains("your Lojban"));
        assert!(fb.contains("corrected Lojban"));
        assert!(fb.contains("1. off"));
    }
...
/// `wasm-pack test --node fanva`. A synthetic result object exercises
...
mod wasm_tests {
    ...
    #[wasm_bindgen_test]
    fn validate_runs_local_gates_on_wasm() {
        // Without the shim, validate() is gerna+smuni — must pass on wasm too.
        assert!(validate("la .adam. cu gerku").is_ok());
    }
}
